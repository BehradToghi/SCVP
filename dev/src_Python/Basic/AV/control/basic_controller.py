...
class Controller2D(object):
    def __init__(self,planner):
        ...
        self._set_steer = 0
        self._waypoints = 0
        # self._waypoints = planner.local_waypoints
        self._conv_rad_to_steer = 180.0 / 70.0 / np.pi
        self._pi = np.pi
        self._2pi = 2.0 * np.pi

    def update_values(self, world_representation, frame):
        x, y, yaw = world_representation.ego_vehicle.localization
        speed = world_representation.ego_vehicle.forward_speed
        timestamp = world_representation.current_timestamp
        self._current_x = x
        self._current_y = y
